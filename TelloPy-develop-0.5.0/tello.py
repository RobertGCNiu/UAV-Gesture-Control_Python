...
class Tello(object):
    ...
    def send(self, message):
        ...
        try:
            self.sock.sendto(message.encode(), tello_address)
    ...
    def quitsimplecontrol(self):
        msg = 'end'
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)



    def landsimplecontrol(self):
        msg = 'land'
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def forwardsimplecontrol(self,val):
        msg = 'forward'+ ' ' + str(val)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def startControlCommand(self):
        msg = 'command'
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def backwardsimplecontrol(self,val):
        msg = 'back'+ ' ' + str(val)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def leftsimplecontrol(self,val):
        msg = 'left'+ ' ' + str(val)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def rightsimplecontrol(self,val):
        msg = 'right'+ ' ' + str(val)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def clockwisesimplecontrol(self,val):
        msg = 'cw'+' '+str(val)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def contourclockwisesimplecontrol(self,val):
        msg = 'ccw'+' '+str(val)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def flip_rightsimplecontrol(self):
        msg = 'flip b'
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def flip_leftsimplecontrol(self):
        msg = 'flip l'
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def takeoffsimplecontrol(self):
        msg = 'takeoff'
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg,self.tello_addr)

    def upsimplecontrol(self,val):
        msg = 'up'+' '+ str(val)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def downsimplecontrol(self,val):
        msg = 'down'+ ' ' + str(val)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)

    def curve_move(self,x1,y1,z1,x2,y2,z2):
        print('ready to curve')
        msg = 'curve'+' '+str(x1)+' '+str(y1)+' '+str(z1)+' '+str(x2)+' '+str(y2)+' '+str(z2)+' '+str(20)
        #speed is 20cm/s
        print('curve x y z done')
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)
        print('curve done')

    def go(self,x,y,z):
        msg='go'+' '+str(x)+' '+str(y)+' '+str(z)+' '+str(30)
        msg = msg.encode(encoding="utf-8")
        self.sock.sendto(msg, self.tello_addr)
